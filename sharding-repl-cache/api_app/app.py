...
SHARD1_URL = os.environ["SHARD1_URL"]
SHARD2_URL = os.environ["SHARD2_URL"]
...
shard1_client = motor.motor_asyncio.AsyncIOMotorClient(SHARD1_URL)
shard1_db = shard1_client[DATABASE_NAME]
shard2_client = motor.motor_asyncio.AsyncIOMotorClient(SHARD2_URL)
shard2_db = shard2_client[DATABASE_NAME]
...
@app.get("/{collection_name}/count/shard1")
async def collection_count1(collection_name: str):
    collection = shard1_db.get_collection(collection_name)
    items_count = await collection.count_documents({})
    # status = await client.admin.command('replSetGetStatus')
    # import ipdb; ipdb.set_trace()
    return {"status": "OK", "mongo_db": DATABASE_NAME, "items_count": items_count}
...
@app.get("/{collection_name}/count/shard2")
async def collection_count2(collection_name: str):
    collection = shard2_db.get_collection(collection_name)
    items_count = await collection.count_documents({})
    # status = await client.admin.command('replSetGetStatus')
    # import ipdb; ipdb.set_trace()
    return {"status": "OK", "mongo_db": DATABASE_NAME, "items_count": items_count}
...
async def get_replica_count(client):
    config = await client.admin.command("replSetGetConfig")
    members = config["config"]["members"]
    return len(members)


@app.get("/shard1/replica-count")
async def replica_count1():
    replica_count = await get_replica_count(shard1_client)
    return {"status": "OK", "replica_count": replica_count}


@app.get("/shard2/replica-count")
async def replica_count2():
    replica_count = await get_replica_count(shard2_client)
    return {"status": "OK", "replica_count": replica_count}
